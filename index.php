...
function gava_edd_register_gateway($gateways){
	$gateways['gava_edd'] = array('admin_label' => 'Gava', 'checkout_label' => __('Pay with EcoCash or TeleCash', 'gava_edd'));
	return $gateways;
}
...
function gava_process_payment($purchase_data)
{
	...
	if (!$payment)
	{
		...
		$hash = $payload['reference'] . $payload['amount'] . $payload['return_url'] . $payload['cancel_url'];
		$hash = hash('sha512', $hash . $edd_options['gava_secret_key']);
